...
class ReactApp @Inject() (protected val dbConfigProvider: DatabaseConfigProvider, cc: MessagesControllerComponents)(implicit ec: ExecutionContext, system: ActorSystem, mat: Materializer)
  ...
  def socket = WebSocket.accept[String, String] { request =>
    ...
    def getProjectByName = Action.async { implicit request =>
        withJsonBody[String] { projectName =>
            println(projectName)
            //val mididata = scala.concurrent.Await.result(model.getProjectByName(projectName), Duration(50000, "millis"))
            //Ok(Json.toJson(mididata(0)))
            Future.successful(Ok(Json.toJson(scala.concurrent.Await.result(model.getProjectByName(projectName), Duration(50000, "millis"))(0))))
        }
